...
public class AnagramChecker<T> {
	...
	public static boolean areAnagrams(String string1, String string2) {
		String s1 = sort(string1.toLowerCase());
		String s2 = sort(string2.toLowerCase());
		return s1.equals(s2);
	}
		...
		for (int i = 0; i < inputArray.length; i += anagramWordsTrack) {
			...
			for (int j = i; j < inputArray.length; j++) {
				if (areAnagrams(inputArray[i], inputArray[j])) {
					anagramWordsTrack++;
					if (anagramWordsTrack > largestAnagramWordCount) {
	...
	public static void printList(String filename) {
		try {
			...
			String[] stringArray = new String[wordList.size()];
			stringArray = wordList.toArray(stringArray);
			insertionSort(stringArray,new Comparing<>());
			System.out.println("");
			System.out.print("Sorted text Array:");
